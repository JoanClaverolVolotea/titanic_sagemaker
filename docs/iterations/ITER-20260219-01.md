...
- Se agrego workflow base para ejecucion del checker en GitHub Actions:
  - `.github/workflows/tutorial-resource-activity-check.yml`
  - soporte para `workflow_dispatch` con `phase` y `fail_if_active`.
...
- Workflow CI:
  - archivo YAML agregado con artefacto `tutorial-resource-activity-report`.
